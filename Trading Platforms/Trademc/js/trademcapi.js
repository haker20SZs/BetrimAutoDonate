...
(function(self){
            ...
            html = '\
            	<div class="trademc-buyform">\
			        ...
			        <form class="trademc-buyform-form">\
				        <input class="form-control" name="buyer" class="trademc-buyform-input" placeholder="' + params.Nickname + '">\
						<br><div><select style="padding: 0.875rem 2rem; font-size: 0.95rem; line-height: 1.5; border-radius: 0.3rem; height: calc(1.5em + 1.75rem + 0.0625rem); background-repeat: no-repeat; background-position: center right calc(0.375em + 0.3rem); background-size: calc(0.4875rem + 0.6rem) calc(0.4875rem + 0.6rem);border-radius: 8px; display: block;" class="form-control" name="items">\
					        	<option selected>' + params.Items + '</option>\
					        	' + html + '\
					    </div></select>\
				        <div class="trademc-buyform-userfields"></div>\
						<br><input class="form-control" name="coupon" class="trademc-buyform-input" placeholder="' + params.Coupon + '">\
		                <br><button style="border-radius: 8px; background-color: #00bfff; font-weight: bold; height: 55px; font-size: 20px;"  class="trademc-buyform-button" type="submit">' + params.Button + '</button>\
    ...
    function getJSON(method, params, callback){
		...
		xhr.onerror = function() {
			callback({ error: { message: "Ошибка 1x762." } });
		}
		xhr.send();
    }
